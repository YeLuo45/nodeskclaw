...
| 执行状态 | MUST | 说明动作执行本身是成功、失败、被拒绝、等待中、部分完成、不支持，还是需要介入 |
...
能力不可用时，返回语义 MUST 让 Agent 理解原因，例如不支持、条件不满足、需要授权、需要更高 Profile 或只能降级执行。这些是 Agent-readable semantic category（Agent 可理解语义类别），不是固定 error enum（错误枚举）、返回字段或状态码。
...
| `Agent-readable semantic contract` | 设备动作必须向 Agent Runtime 和 Context Controller 表达的可理解语义，不等同于固定返回结构或统一 wire format |
...
5. 设备动作结果能表达操作意图、执行状态、状态变化、失败原因、外部副作用和审计证据，并在可判断时表达下一步建议；失败、拒绝、部分完成或状态未知时能表达最小失败语义。
...
| 成功和失败语义 | Provider 返回中的成功、失败、完成、拒绝或等待等判断必须绑定到明确语义维度；没有维度归属的泛化 `success` / `failure` 不构成合规语义契约 |
